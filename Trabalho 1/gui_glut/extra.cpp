...
void glutGUI::mouseMove(int x, int y) {
    ...
    if (!lbpressed && rbpressed && !mbpressed) {
        ...
        /* Se (trans_luz) habilitado */
        if (trans_luz) {
            lx += (x - last_x)/fator;
            ly += -(y - last_y)/fator;
        }
    }
    /* Se botão Esquerdo e Direito */
    if (lbpressed && rbpressed && !mbpressed) {
        if (!trans_obj && !trans_luz) {
            cam->zoom(y,last_y);
        }
